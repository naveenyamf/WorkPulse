...
router.post('/run', async (req, res) => {
  ...
  try {
    ...
    const tables = [
      ...
      { name: 'employees',            sql: `CREATE TABLE IF NOT EXISTS employees (id SERIAL PRIMARY KEY, name VARCHAR(200) NOT NULL, email VARCHAR(200) UNIQUE NOT NULL, department VARCHAR(100), agent_token VARCHAR(100) UNIQUE NOT NULL, active BOOLEAN DEFAULT true, screenshot_interval INTEGER DEFAULT 5, data_retention_days INTEGER DEFAULT 0, roster_id INTEGER REFERENCES duty_rosters(id) ON DELETE SET NULL, machine_id VARCHAR(255), deleted_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT NOW())` },
      { name: 'heartbeats',           sql: `CREATE TABLE IF NOT EXISTS heartbeats (id SERIAL PRIMARY KEY, employee_id INTEGER NOT NULL REFERENCES employees(id) ON DELETE CASCADE, active_app VARCHAR(500), idle BOOLEAN DEFAULT false, agent_version VARCHAR(20), recorded_at TIMESTAMPTZ DEFAULT NOW())` },
